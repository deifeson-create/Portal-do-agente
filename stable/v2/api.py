...
# ==============================================================================
# FUNÇÕES ESPECÍFICAS DO AGENTE (VISÃO INDIVIDUAL)
# ==============================================================================
        ...
        while True:
            ...
            try:
                ...
                for bloco in data:
                    ...
                    if id_perg == str(PERGUNTA_TEXTO): continue 
            ...
            if r_global.status_code == 200:
                ...
                if lista_global and isinstance(lista_global, list):
                    for item in lista_global:
                        ...
                        cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] == nome_api), None)
                        if not cod_match: cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] in nome_api or nome_api in mapa_agentes[c]), None)
                        if cod_match:
                        ...
                        if id_perg in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for resp in bloco.get("respostas", []):
                                try:
                                    ...
                                    cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] == nom_ag), None)
                                    if not cod_match: cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] in nom_ag or nom_ag in mapa_agentes[c]), None)
                                    if cod_match:
...
def _processar_agente_pausas(token, cod_agente, nome_agente, data_ini, data_fim):
    ...
    for p in pausas_agente:
        ...
        if any(x in motivo for x in ["MANHA", "MANHÃ", "TARDE", "NOITE"]):
            limite_aplicado = LIMITES_PAUSA["CURTA_ANTIGA"]
            tipo_pausa = "CURTA"
        elif any(x in motivo for x in ["PAUSA 1", "PAUSA 3"]):
            limite_aplicado = LIMITES_PAUSA["PAUSA_1_3"]
            tipo_pausa = "CURTA"
        elif any(x in motivo for x in ["PAUSA 2"]):
            limite_aplicado = LIMITES_PAUSA["PAUSA_2"]
            tipo_pausa = "LONGA" 
        elif any(x in motivo for x in ["ALMOÇO", "ALMOCO", "PLANTÃO", "PLANTAO"]):
            limite_aplicado = LIMITES_PAUSA["LONGA_ANTIGA"]
            tipo_pausa = "LONGA"
            
        if tipo_pausa == "CURTA":
            if minutos > limite_aplicado:
                acumulado_excesso_curta += (minutos - limite_aplicado)
        ...
        try:
            ...
            if r.status_code == 200:
                for item in r.json():
                    ...
                    if not cod_match:
                        cod_match = next((c for c in ids_validos if mapa_agentes[c] in nome_api or nome_api in mapa_agentes[c]), None)
                        ...
                        if id_perg in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for rsp in b.get("respostas", []):
                                try:
                                    ...
                                    if not cod_match:
                                        cod_match = next((c for c in ids_validos if mapa_agentes[c] in nom_ag or nom_ag in mapa_agentes[c]), None)
...
def buscar_dados_plantao(token, data_ini, data_fim, contas_selecionadas):
    ...
    for conta in contas_selecionadas:
        params_agente = {
            "data_inicial": f"{data_ini.strftime('%Y-%m-%d')} 00:00:00",
            "data_final": f"{data_fim.strftime('%Y-%m-%d')} 23:59:59",
            "agrupador": "agente", "agente[]": ids_plantao, "canal[]": ids_canais, "id_conta": conta
        }
        try:
            ...
            if r.status_code == 200:
                for item in r.json():
                    ...
                    if not cod_match: cod_match = next((c for c in ids_plantao if mapa_plantao[c] in nome_api or nome_api in mapa_plantao[c]), None)
                    if cod_match:
    ...
    for conta in contas_selecionadas:
        params_servico = {
            "data_inicial": f"{data_ini.strftime('%Y-%m-%d')} 00:00:00",
            "data_final": f"{data_fim.strftime('%Y-%m-%d')} 23:59:59",
            "agrupador": "servico", "agente[]": ids_plantao, "canal[]": ids_canais, "id_conta": conta
        }
        try:
                    ...
                    for b in dd:
                        ...
                        if id_perg in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for rsp in b.get("respostas", []):
                                ...
                                if not cod_match: cod_match = next((c for c in ids_plantao if mapa_plantao[c] in nom_ag or nom_ag in mapa_plantao[c]), None)
                                if cod_match:
...
def buscar_dados_cliente_interno(token, data_ini, data_fim, ids_suporte_validos):
    ...
    try:
        params = {
            "data_inicial": f"{data_ini.strftime('%Y-%m-%d')} 00:00:00", "data_final": f"{data_fim.strftime('%Y-%m-%d')} 23:59:59",
            "agrupador": "conta", "canal[]": ids_canais, "id_conta": ID_CONTA_CLIENTE_INTERNO 
        }
    ...
    for p_id in PESQUISAS_IDS:
        pg = 1
        while True:
            params_p = {"data_inicial": data_ini.strftime("%Y-%m-%d"), "data_final": data_fim.strftime("%Y-%m-%d"), "pesquisa": p_id, "id_conta": ID_CONTA_CLIENTE_INTERNO, "limit": 1000, "page": pg}
            try:
                ...
                for bloco in data:
                    ...
                    if id_perg in IDS_PERGUNTAS_VALIDAS:
                        ...
                        for resp in bloco.get("respostas", []):
                            ...
                            eh_do_suporte = any(alvo == nome_agente_resp or alvo in nome_agente_resp for alvo in nomes_suporte)
                            if eh_do_suporte:
                                val_raw = resp.get("nom_valor")
...
def buscar_dados_jovem_aprendiz(token, data_ini, data_fim, setor_nome, contas_selecionadas):
    ...
    for conta in contas_selecionadas:
        try:
            params_g = {
                "data_inicial": f"{data_ini.strftime('%Y-%m-%d')} 00:00:00", "data_final": f"{data_fim.strftime('%Y-%m-%d')} 23:59:59",
                "agrupador": "conta", "agente[]": ids_ja, "canal[]": ids_canais, "id_conta": conta
            }
    ...
    for conta in contas_selecionadas:
        try:
            params_a = {
                "data_inicial": f"{data_ini.strftime('%Y-%m-%d')} 00:00:00", "data_final": f"{data_fim.strftime('%Y-%m-%d')} 23:59:59",
                "agrupador": "agente", "agente[]": ids_ja, "canal[]": ids_canais, "id_conta": conta
            }
            ...
            if r_a.status_code == 200:
                for item in r_a.json():
                    ...
                    if not cod_match: cod_match = next((c for c in ids_ja if mapa_ja[c] in nome_api or nome_api in mapa_ja[c]), None)
                    if cod_match:
                        vol = int(item.get("num_qtd", 0)) - int(item.get("num_qtd_abandonado", 0))
                        if vol > 0:
    ...
    for p_id in PESQUISAS_IDS:
        for conta in contas_selecionadas:
            pg = 1
            while True:
                params_p = {"data_inicial": data_ini.strftime("%Y-%m-%d"), "data_final": data_fim.strftime("%Y-%m-%d"), "pesquisa": p_id, "id_conta": conta, "limit": 1000, "page": pg, "agente[]": ids_ja}
                try:
                    ...
                    for bloco in data:
                        ...
                        if id_perg in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for resp in bloco.get("respostas", []):
                                ...
                                if not cod_match: cod_match = next((c for c in ids_ja if mapa_ja[c] in nom_ag or nom_ag in mapa_ja[c]), None)
                                if cod_match:
                                    val_raw = resp.get("nom_valor")
                                    if val_raw and val_raw != "": 
                                        nota = int(float(val_raw))
...
def buscar_dados_satisfacao(token, data_ini, data_fim, contas_selecionadas, mapa_agentes):
    ...
    for conta in contas_selecionadas:
        page = 1
        while True:
            params = {"data_inicial": data_ini.strftime("%Y-%m-%d"), "data_final": data_fim.strftime("%Y-%m-%d"), "pesquisa": PESQUISA_SATISFACAO, "id_conta": conta, "limit": 1000, "page": page, "agente[]": ids_validos}
            try:
                r = requests.get(url, headers=headers, params=params, timeout=30)
                time.sleep(0.5)
                if r.status_code != 200: break
                ...
                for bloco in data:
                    ...
                    for resp in respostas:
                        ...
                        nom_ag = normalizar_nome_pesquisa(resp.get("nom_agente", ""))
                        cod_match = next((c for c in ids_validos if mapa_agentes[c] == nom_ag or mapa_agentes[c] in nom_ag or nom_ag in mapa_agentes[c]), None)
                        if cod_match:
                            ...
                            if id_perg == PERGUNTA_NOTA:
                                val_raw = resp.get("nom_valor")
                                if val_raw and val_raw != "":
                                    try: protocolos[proto]["Nota"] = int(float(val_raw))
                                    except: pass
                            elif id_perg == PERGUNTA_TEXTO:
                ...
                if (page * 1000) >= total_api and encontrou_pergunta: break
                if len(data) < 2 and not encontrou_pergunta: break
                page += 1
            except: break
...
def buscar_csat_unificado_suporte(token, data_ini, data_fim, contas_selecionadas):
    """Calcula matematicamente o CSAT absoluto de todo o Suporte (Dia + Plantão + Aprendizes)"""
    headers = {"Authorization": f"Bearer {token}"}
    url = f"{BASE_URL}/RelPesqAnalitico"
    
    # Consolida a lista inteira de IDs do ecossistema do Suporte
    todos_ids_suporte = list(set(SETORES_AGENTES_IDS.get("SUPORTE", []) + LISTA_PLANTAO_IDS + JOVENS_APRENDIZES_SUPORTE_IDS))
    
    # Como não temos o mapa_agentes aqui, vamos buscar os nomes autorizados para blindar a API
    nomes_autorizados = []
    pagina = 1
    while True:
        try:
    ...
    for p_id in PESQUISAS_IDS:
        for conta in contas_selecionadas:
            ...
            while True:
                params = {
                    "data_inicial": data_ini.strftime("%Y-%m-%d"), "data_final": data_fim.strftime("%Y-%m-%d"),
                    ...
                    for bloco in data:
                        ...
                        if id_perg in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for resp in bloco.get("respostas", []):
                                ...
                                # Verifica se o nome traduzido pertence à equipe de Suporte
                                eh_da_equipe = False
                                for nome_auth in nomes_autorizados:
                                    if nom_ag == nome_auth or nom_ag in nome_auth or nome_auth in nom_ag:
                                        eh_da_equipe = True
                                        break
