...
# Importando as variáveis de configuração e funções úteis que acabamos de separar
from config import (
            ...
            if r_global.status_code == 200:
                ...
                if lista_global and isinstance(lista_global, list):
                    for item in lista_global:
                        ...
                        if not cod_match:
                            cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] in nome_api or nome_api in mapa_agentes[c]), None)
                    ...
                    for bloco in data:
                        if str(bloco.get("id_pergunta", "")) in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for resp in bloco.get("respostas", []):
                                try:
                                    nom_ag = str(resp.get("nom_agente", "")).strip().upper()
                                    cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] == nom_ag), None)
                                    if not cod_match:
                                        cod_match = next((c for c in ids_agentes_stats if mapa_agentes[c] in nom_ag or nom_ag in mapa_agentes[c]), None)
...
def buscar_dados_supervisor_multisetor(token, data_ini, data_fim, setor_nome, contas_selecionadas, excluir_plantao=False):
    ...
    for cod in ids_agentes:
        if excluir_plantao and setor_nome == "SUPORTE" and cod in LISTA_PLANTAO_IDS:
            continue
        if cod in ids_aprendizes:
            continue
                    ...
                    for b in dd:
                        if str(b.get("id_pergunta","")) in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for rsp in b.get("respostas", []):
                                try:
                                    nom_ag = str(rsp.get("nom_agente", "")).strip().upper()
        ...
        try:
            ...
            if r.status_code == 200:
                for item in r.json():
                    ...
                    if not cod_match:
                        cod_match = next((c for c in ids_plantao if mapa_plantao[c] in nome_api or nome_api in mapa_plantao[c]), None)
                    ...
                    for b in dd:
                        if str(b.get("id_pergunta","")) in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for rsp in b.get("respostas", []):
                                ...
                                if not cod_match:
                                    cod_match = next((c for c in ids_plantao if mapa_plantao[c] in nom_ag or nom_ag in mapa_plantao[c]), None)
                ...
                for bloco in data:
                    if str(bloco.get("id_pergunta", "")) in IDS_PERGUNTAS_VALIDAS:
                        ...
                        for resp in bloco.get("respostas", []):
                            ...
                            eh_do_suporte = False
                            for alvo in nomes_suporte:
                                if alvo == nome_agente_resp or alvo in nome_agente_resp:
                                    eh_do_suporte = True; break
        ...
        try:
            ...
            if r_a.status_code == 200:
                for item in r_a.json():
                    ...
                    if not cod_match:
                        cod_match = next((c for c in ids_ja if mapa_ja[c] in nome_api or nome_api in mapa_ja[c]), None)
                    ...
                    for bloco in data:
                        if str(bloco.get("id_pergunta", "")) in IDS_PERGUNTAS_VALIDAS:
                            ...
                            for resp in bloco.get("respostas", []):
                                ...
                                if not cod_match:
                                    cod_match = next((c for c in ids_ja if mapa_ja[c] in nom_ag or nom_ag in mapa_ja[c]), None)
